...
基于 `python-telegram-bot` v22 开发的异步投稿机器人，允许用户向机器人私聊投稿，由指定审核群的管理员进行审核，并通过机器人发布到指定频道。
...
4.  **配置 `config.json`**:
    *   至少填写以下必要信息：
        ```json
        {
            ...
            "Admin": 123456789,       // 你的 Telegram User ID (机器人管理员)
...
6.  **初始化设置**:
    *   启动机器人后，管理员需要将机器人添加到**稿件审核群**后发送 `/setgroup` 命令。
    *   管理员通过私聊或在任意群组发送 `/setchannel @频道用户名` 或 `/setchannel 频道数字ID（通常以-100开头）` 命令设置发布频道。
    *   确保机器人已被添加到稿件审核群和稿件发布频道，并在稿件发布频道拥有管理员权限。
